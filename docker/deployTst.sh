#!/bin/bash -xe
# Deploy docker image to TST environment.
...
# pharmaledger at acdc-tst-pl
USERATHOST="pharmaledger@192.168.13.105"
...
docker run --detach --hostname $UCNAME --network="host" --publish 8080:8080 --mount source=external-volume,target=/auth-feature-workspace/apihub-root/external-volume --name $UCNAME --restart always pharmaledger/$UCNAME
